       ...
       <div class="flex items-center justify-end gap-1" >
         <IconComponent @click="setThemeMode()" >
           ...
           <IconComponent class="relative hover:bg-lightPrimary " >
              <span class="absolute bottom-6 font-semibold -right-0 bg-[#e82968] rounded-full flex items-center justify-center w-4 h-4" ><span class="text-white mt-[1px] text-xs" >0</span></span>
              <NotificationDuoIcon class="w-7 h-7" />
            </IconComponent>
            <IconComponent>
                <SettingsDuoIcon class="w-7 h-7"/>
            </IconComponent>
            <ImageComponent rounded alt="agent profile pic" :src="image1" class="max-w-7 max-h-7 cursor-pointer" /> 
       </div>
    </div>
</template>